...
```mermaid
flowchart TB
    subgraph AA["ADOPTION ARCHITECT"]
        subgraph Clarity["CLARITY"]
            C1[Aggregate signals]
            C2[Identify patterns]
            C3[Define cohorts]
            C4[Publish insights]
        end
        subgraph Action["ACTION"]
            A1[Run platform programs]
            A2[Create enablement]
            A3[Escalate to PM]
            A4[Guide account teams]
        end
    end
    
    Clarity --> Playbooks["REPEATABLE PLAYBOOKS<br/>Scale adoption across BU+1s"]
    Action --> Playbooks
```
